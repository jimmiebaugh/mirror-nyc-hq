...
See `CHECKPOINT.md` for live state. As of this writing: **Phase 4.10.4-port (pre-cutover smoke polish)** squashed onto `vs-port-fresh` at `<TBD-4.10.4-port-squash>` (prior 4.10.3-port at `9bc27e8`, 4.10.2-port at `a36eb3d`, 4.10.1-port at `1ac3f6f`). 10 items: rank hidden from matrix render (DB + tool emission + patch-write paths stay; reversible UI hide), secondary sort flipped to alphabetical-by-name with numeric collation; Upload Photos column dropped from Shortlist (photos live on Review only); per-row Notes / Feedback textarea on Review bound to `vs_candidate_venues.notes` via debounceSave + coral descriptor + page-intro descriptor sentence; `vs-compile-summaries` `OVERVIEW_TOOL` tuned for 3-4 sentence (~80 word) overviews via tool description + property description + `maxLength: 600` (`OVERVIEW_SYSTEM` stays frozen per `feedback_tool_choice_collapse`); Compiling + Generating loading copy refined; Deck Prep bottom nav reworked (slide-count line dropped, "X Venues" bumped to text-xl, bulleted included-venue list); Deck Prep notes consolidated into a single bulleted card above the table; Deck Prep cell line-breaks for neighborhood / size / capacity. No migrations. vs-compile-summaries deploy verified via `supabase functions list` (version 17, 2026-05-13 19:29:46 UTC). Out-of-band pre-work: vs-generate-deck redeploy cleared a 4.10.3 stale-deploy production failure. **VS Pro port is feature-complete and cutover-ready.** Next: **cutover** per `docs/venue-scout-port-plan.md` § "Done when" (cherry-pick `f24d3f5` must-carry set, verify via `git show --stat` diff equivalence, hard-reset main to vs-port-fresh HEAD; fires Netlify deploy). The failed-attempt Phase 4 stack on `main` (Scout Dashboard through Deck Prep + URL-quality hot patch) is archived; do not extend it.
