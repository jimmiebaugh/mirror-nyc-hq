...
See `CHECKPOINT.md` for live state. As of this writing: **Phase 4.8.1-port (Deck Prep + googleServiceAccount cherry-pick + gmailServiceAccount delegation refactor)** squashed onto `vs-port-fresh` at `<TBD-4.8.1-port-squash>`. **Phase 4.8.2-port (Generating + vs-generate-deck + secrets verification)** is next per `docs/venue-scout-port-plan.md`. The failed-attempt Phase 4 stack on `main` (Scout Dashboard through Deck Prep + URL-quality hot patch) is archived; do not extend it.
