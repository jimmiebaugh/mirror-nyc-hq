...
See `CHECKPOINT.md` for live state. As of this writing: **Phase 4.5-port (Researching + vs-research-venues)** done on the `vs-port-fresh` branch; **Phase 4.6-port (Sourcing Report + Shortlist + matrix primitives)** is next per `docs/venue-scout-port-plan.md`. The failed-attempt Phase 4 stack on `main` (Scout Dashboard through Deck Prep + URL-quality hot patch) is archived; do not extend it.
