...
See `CHECKPOINT.md` for live state. As of this writing: **Phase 4.4-port (Sheet Prompt + Sheet Upload + parse-sheet)** done on the `vs-port-fresh` branch; **Phase 4.5-port (Researching + research-venues)** is next per `docs/venue-scout-port-plan.md`. The failed-attempt Phase 4 stack on `main` (Scout Dashboard through Deck Prep + URL-quality hot patch) is archived; do not extend it.
