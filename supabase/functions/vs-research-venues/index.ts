...
//      sanitizeWebsiteUrl from _shared/venueTypes.ts (rejects search pages
//      + listing-DB homepages, lets deep links through).
...
//      on vs_scouts (or status='failed' + research_error on failure).
//
        ...
        items: {
          ...
          properties: {
            ...
            website_url: {
              type: "string",
              description:
                "URL for this venue's individual detail page. This is the URL a producer clicks to see THIS specific venue's photos, square footage, and rental info. Examples: the venue's own dedicated website (e.g. https://parasolprojects.com), or its dedicated listing page on a platform (e.g. peerspace.com/spaces/12345, thestorefront.com/listing/west-hollywood-pop-up, splacer.co/spaces/sunset-loft, tagvenue.com/venue/12345-rooftop-bar-la, partyslate.com/venues/the-london-west-hollywood).",
            },
...
Return ${targetNet} net-new venue candidates (10-15 total considering the existing list). Weight ground-floor commercial / storefront / retail-vacancy spaces heavily in ranking_score. Pick 3-4 derived columns that map to the most decision-relevant brief criteria.`;
  ...
  const work = async () => {
    try {
      // 120-second ceiling via Promise.race. Defense-in-depth so a
      // hanging Anthropic call doesn't leave the page spinning forever.
      // Capture the handle so we can clearTimeout on normal completion;
      // otherwise the orphaned timer keeps EdgeRuntime.waitUntil alive
      // for the full 120s even when Claude returns in 30.
      ...
      const callPromise = callClaude(
        ...
        [{ role: "user", content: userMsg }],
        {
          ...
          // diagnostic log shows out<200 AND server_tool_uses=0 on first
          // round, pivot to claude-sonnet-4-5 by adding
          //   model: "claude-sonnet-4-5",
          // here. Update docs/decisions.md + CHECKPOINT.md + memory
          // note project_sourcing_model_pin at the same time.
          max_tokens: 8000,
          system: SYSTEM,
          tools: [
            TOOL,
      ...
      // through canonicalizeType; URL goes through sanitizeWebsiteUrl.
      ...
      const cleanVenues = venues
        .map((raw: unknown) => {
          ...
          return {
            ...
            venue_type,
            website_url: sanitizeWebsiteUrl(v.website_url),
            size_sq_ft:
