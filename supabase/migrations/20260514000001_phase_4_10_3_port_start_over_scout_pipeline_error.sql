-- Phase 4.10.3-port: update start_over_scout RPC to clear pipeline_error
-- instead of research_error. CREATE OR REPLACE; body otherwise unchanged
-- from the 4.9-port migration.
...
COMMENT ON FUNCTION public.start_over_scout IS
  'Phase 4.9-port (updated 4.10.3-port for pipeline_error rename): reset a scout back to sheet_prompt. Cascade-deletes vs_candidate_venues (photos cascade via FK). Keeps brief fields, project_id, generated_decks history, brief_data.uploaded_files. Resets current_step, status, pipeline_error, derived_columns, sheet_storage_path, deck_order, brief_data idempotency timestamps. SECURITY INVOKER.';
