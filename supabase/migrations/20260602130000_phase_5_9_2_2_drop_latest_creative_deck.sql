-- Phase 5.9.2.2: drop the unused projects.latest_creative_deck_url column.
--
-- The column shipped in the Phase 5 initial schema but was never surfaced in
-- any UI (no ProjectDetail / ProjectEdit reference). The only writer was the
-- 5.9.2 importer, which has been updated to stop sending it. Steps:
--   1. CREATE OR REPLACE bulk_import_commit_projects without the column (a
--      plpgsql body referencing a dropped column would fail at next call).
--   2. DROP the column.
--
-- Destructive (DROP COLUMN) but authorized: the column is unused and carries no
-- surfaced data. Function body is otherwise identical to 5.9.2.1.
  ...
  LOOP
    ...
    ELSE
      INSERT INTO public.projects (
        ...
        budget_sheet_url, slack_channel_url,
...
ALTER TABLE public.projects DROP COLUMN IF EXISTS latest_creative_deck_url;
