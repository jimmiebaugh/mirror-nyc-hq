-- Phase 5.9.7: Import Event Day Rate + venues.general_email.
--
-- Two related additions ship together (one squash):
--   A. venues.general_email (nullable text). Company-level email distinct from
--      any specific contact, mirroring the 5.9.3.2 vendors.general_email shape.
--   B. The venue commit RPC learns to (1) append one event_day venue_rate_history
--      row per imported venue dated current_date, only when the amount differs
--      from the venue's current most-recent event_day rate (the table is
--      append-only; this keeps a repeat import a no-op), and (2) write
--      general_email through on both the create + update paths.
--
-- COORDINATION (spec §14): this CREATE OR REPLACE is rebased onto the 5.9.6
-- undo body (20260602200000), which already persists imported_record_ids +
-- imported_person_ids for the undo trail. Both changes compose: 5.9.6 tracks
-- the created records / created-only contact People; 5.9.7 adds the rate write
-- + the general_email field. Neither is lost.
--
-- Reversibility: additive. The ALTER adds a nullable column; the function is a
-- CREATE OR REPLACE on an existing SECURITY DEFINER RPC (a down-migration would
-- need the prior 5.9.6 body). Flagged in the PR description per docs/conventions.
--
...
-- A. Add the nullable column. Same shape as 5.9.3.2 vendors.general_email.
ALTER TABLE public.venues
  ADD COLUMN general_email text;

-- B. Replace the venue commit RPC. Body is the 5.9.6 version plus the rate
--    write block + general_email write-through.
