...
export default function DeliverableDetail() {
  ...
  const detailMeta = [
    ...
    row.due_date ? formatShortDate(row.due_date) : null,
  ].filter(Boolean);
      ...
      <header className="stack-3">
        ...
        <div className="row between" style={{ alignItems: "center" }}>
          <div className="row-c" style={{ flex: 1, gap: 16, alignItems: "center", minWidth: 0, flexWrap: "wrap" }}>
            <h1 className="h-page" style={{ minWidth: 0 }}>{row.title || "(untitled)"}</h1>
            <ClickPillCell
              value={row.status}
      ...
      <section className="card">
        ...
        <div className="card-pad stack-4">
          ...
          <DField label="Assignee">
            {/* Phase 5.11.2: render multi-assignee as a coral text stack
                (combo-stack with no href) so the field reads as
                RecordCombobox text instead of pill chips. Matches Task
                Detail's single-assignee text presentation. */}
            <RecordCombobox
              multi
              displayAs="stack"
          ...
          {row.completed_at ? (
            <>
              ...
              <DField label="Completed">
                <span>{formatMediumDate(row.completed_at.slice(0, 10))}</span>
              </DField>
            </>
          ) : null}
        </div>
