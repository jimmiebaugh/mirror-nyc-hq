// Default evaluation prompt template — mirrors supabase/functions/_shared/evalPrompt.ts.
// (Duplicated because edge functions can't import from src/.)
//
// This prompt is used as the SYSTEM block for per-candidate Claude calls and is
// cached (1-hour TTL) by the Anthropic prompt cache. Role context and candidate
// materials are sent separately in the user message — do not embed per-call
...
// Legacy helper retained for any UI surfaces that preview a filled template.
export function fillEvalPrompt(template: string, vars: {
  role_title?: string;
  jd?: string;
  ...
  const map: Record<string, string> = {
    role_title: vars.role_title ?? "",
    jd: vars.jd ?? "",
    hiring_priorities: vars.hiring_priorities ?? "",
    threshold: String(vars.threshold ?? ""),
    scorecard: vars.scorecard ?? "",
    competitors: vars.competitors ?? "",
    candidate_bundle: vars.candidate_bundle ?? "",
    detected_urls: vars.detected_urls ?? "",
  };
