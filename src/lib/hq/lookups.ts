 ...
 *
 * The four shipped lookups: `cities`, `project_categories`,
 * `org_capabilities`, `venue_types`. All share the same shape (id uuid,
 * name text, created_by uuid, created_at) and the same open-authenticated
 * SELECT/INSERT RLS posture. `venue_types` predates the 5.2.2 lookups but
 * fits the same hook signature; consuming it through `useLookup` keeps the
 * Venue Edit's Venue-Type inline-add consistent with the other three.
 *
