...
**Active feature branch:** `vs-port-fresh` — branched off `dd38577` (last Cowork-side doc commit before the failed Phase 4 shipped). Accumulates the 1:1 port from `mirror-nyc-venue-scout-pro` per `docs/venue-scout-port-plan.md`.
**Active sub-phase worktree:** `claude/vs-port-4-1-schema` at `.claude/worktrees/busy-dewdney-898b26`.
**Latest commit on `vs-port-fresh`:** `ae22bb8` (port plan landing; Phase 4.1-port follows in the next commit on the worktree branch).
**Current phase:** Phase 4.1-port DONE; next: Phase 4.2-port (Scout Index + New Scout entry).
...
## Recent commits (claude/vs-port-4-1-schema worktree, NOT on vs-port-fresh yet)

Phase 4.1-port commit pending. Squashes to one commit on `vs-port-fresh` after Jimmie's approval.
