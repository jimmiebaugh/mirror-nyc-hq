...
**Latest commit on `main`:** `20e17ec` (Phase 4 Revision: 3-step brief intake stepper). Rebuilds the Venue Scout Brief from a single page into a 3-step stepper (Event -> Venue -> Review), adds the venue-side fields the AI sourcing prompt needs, the `vs-generate-brief-overview` edge function, and the `brief` `current_step` value. Squash-merge of `claude/phase-4-revision-intake` (7 worktree commits incl. local smoke-test feedback). Fires the Netlify deploy that ships the intake UI. Migration `20260514110000` + the three edge function deploys (`vs-generate-brief-overview` new, `vs-parse-brief` + `vs-research-venues` redeployed) were applied to `amipjjmphblfxpghjnel` out-of-band ahead of the merge. Followed by an administrative CHECKPOINT.md backfill commit (`[skip netlify]`).
**Active feature branch:** none. `claude/phase-4-revision-intake` was squash-merged + its worktree removed. Future work spawns its own branches.
...
- **Phase 4 Revision - Intake** (`20e17ec`, shipped 2026-05-14): the Venue Scout Brief is now a 3-step stepper. `New Scout -> /brief` redirect index dispatches to `/brief/event` (Step 1: brief PDF upload + event details), `/brief/venue` (Step 2: venue-side fields -- target neighborhoods + strict toggle, venue-type chips, square-footage sliders, ideal features, priority toggles; `city` is now required), `/brief/report` (Step 3 + permanent Brief surface: inline-editable cards, hero band, Generated Decks section, auto-generated Event Overview via `vs-generate-brief-overview`). New `current_step` value `brief` (migration `20260514110000`, also flips the new-row default to `brief`). The Revisit nav (`ScoutStepThroughNav`) is always-visible now with active/reached/unreached chip states. `vs-parse-brief` parses the new fields; `vs-research-venues` Phase B injects them into the sourcing prompt. All 16 new fields persist to `vs_scouts.brief_data` jsonb (no column migration). Spec: `OUTPUTS/phase-4-revision-intake-spec.md`; decisions in `docs/decisions.md` § Phase 4 Revision.
...
- **Phase 4 Revision deferred items** (not blocking): defense-in-depth sanitization of producer free-text before Claude prompt interpolation in `vs-research-venues` / `vs-generate-brief-overview` (uniform pass, should also cover the pre-existing `brief_data.notes` injection); `SCOUT_SELECT` column string duplicated across the three Brief pages. Both flagged by security-auditor / code-reviewer and recorded in the squash commit message.

## Recent commits (main)

```
20e17ec  Phase 4 Revision: 3-step brief intake stepper
00536d0  [skip netlify] Add triage-observations skill + slash command
