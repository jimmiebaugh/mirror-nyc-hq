...
- Talent Scout is fully wired through Phase 3.7 for admin users:
  ...
  - **Global Settings** (admin only) under `/talent-scout/settings`: global competitor list edit (Postgres `text[]`, seeded with Mirror's 19-entry default), default applicable to all roles unless overridden per-role.
...
- No cron jobs running yet (Phase 3.8).
...
- `monthly-spend-reset` cron not implemented yet — `cap_alert_sent_this_month` will not auto-reset on month boundary. Manual SQL reset works in the meantime (see `docs/operations.md`). Lands in Phase 3.8.
...
20260506213340_phase_3_6_final_review_and_packets.sql     Phase 3.6 — ts_final_reviews enrichment, ts_pull_rounds.packet_*, ts_candidates.email_body_text, packets bucket, realtime publication (APPLIED)
```

## Next up

**Phase 3.8: Cron + watchdogs.** Per `docs/roadmap.md`: `ts-cron-scheduled-pulls`, `ts-cron-pull-watchdog`, `ts-cron-reeval-watchdog`, `ts-cron-final-review-watchdog`, `ts-cron-storage-cleanup`. Plus `monthly-spend-reset` cron and a real cap-alert email sender (currently a console.log stub in `_shared/anthropic.ts`). Cut a fresh `phase-3-8-cron-watchdogs` branch.

**Phase 3.X cleanup queued for after merge:**
- Drop `ts_pull_rounds.reeval_last_progress_at` (dead column).
