...
**Latest commit on `vs-port-fresh`:** `a31a73a` (Phase 4.10.6-port squashed). Prior: `1287478` (Phase 4.10.5-port), `50cba30` (Phase 4.10.4-port), `9bc27e8` (Phase 4.10.3-port), `a36eb3d` (Phase 4.10.2-port), `1ac3f6f` (Phase 4.10.1-port).
**Current phase:** **Phase 4.10.6-port DONE** on `vs-port-fresh`; URL acquisition fallbacks + deck flow polish shipped. Per `docs/decisions.md` § Phase 4.10.6-port for the full list. Key items: new `extractWebSearchResults` + `findVenueWebsite` + `findBestSearchResultUrl` helpers harvest URLs directly from `web_search_tool_result` blocks (or via targeted per-venue Claude calls) when the model leaves `website_url` null; `submit_research.name` schema tightened to forbid descriptive suffixes; `address` + `neighborhood` added to `FILL_TOOL` so Phase A can fill addresses when producer leaves them blank; post-deck-generation flow opens deck in new tab + returns producer to Deck Prep; new `reset_scout_for_deck_regenerate` RPC (migration `20260514100000`) for atomic regenerate state reset; slide 2 ALL CAPS via scoped pre-pass; per-slide `updateSlidesPosition` for canonical `[V1_detail, V1_fp, V2_detail, V2_fp, ...]` venue slide order; vs-generate-deck success-path CAS guard; photo dnd switched to `rectSortingStrategy`; "Contact the team" ghost button removed from 4 deck-error configs. **Phase 4.10.5-port** (one commit prior) restored AI surface stability: model + web_search pivot to `claude-sonnet-4-6 + web_search_20250305`; `pause_turn` continuation loop in `callClaude`; `writeFailure` CAS guards on all 3 AI edge functions; timeout sizing for Pro plan (WORK_TIMEOUT_MS 360s); `brief_data` JSON dump removed from Claude user messages; placeholder string sanitizer; drop forced `tool_choice` on Phase A. One new migration applied (`20260514100000`). vs-research-venues v24, vs-compile-summaries v29, vs-generate-deck v13, vs-parse-sheet v18, vs-parse-brief v14 — all redeployed and verified via `supabase functions list`. **VS Pro port is feature-complete and cutover-ready.** Next: cutover sequence per port plan § "Done when" (cherry-pick `f24d3f5` from main onto vs-port-fresh, verify clean apply via `git show --stat`, hard-reset main). Phase 4.7 was split into two passes (both shipped). Phase 4.8 was split into 4.8.1 / 4.8.2 + 4.8.3 hotfix (all shipped). 4.9 landed Settings page + full ErrorState.tsx + post-completion step-through nav + always-visible gear icon. 4.10 split into six polish sub-phases (4.10.1 enrichment + 4.10.2 matrix overhaul + 4.10.3 final polish + 4.10.4 smoke polish + 4.10.5 AI surface stabilization + 4.10.6 URL fallbacks + deck flow polish; all shipped).
...
```
a31a73a  [skip netlify] Phase 4.10.6-port: URL acquisition fallbacks + deck flow polish (squashed from post-4.10.4 hot patch rounds 13-20 + code-review fixes)
1287478  [skip netlify] Phase 4.10.5-port: AI surface stabilization (squashed from post-4.10.4 hot patch rounds 1-12 + Researching copy tweak)
50cba30  [skip netlify] Phase 4.10.4-port: pre-cutover smoke polish — rank hidden in matrix + alphabetical secondary sort + photo column dropped from Shortlist + Notes/Feedback editor on Review + OVERVIEW_TOOL tuning + Compiling/Generating copy + Deck Prep bottom-nav rework + Deck Prep notes consolidated above + Deck Prep cell line-breaks (squash-merged from claude/vs-port-4-10-4-smoke-polish)
9bc27e8  [skip netlify] Phase 4.10.3-port: URL HEAD validation + Recs/Considerations schema tuning + 3-tier source sort + venue_type popover + pipeline_error rename + VS storage policy reconciliation + per-venue enrichment progress + side-by-side reconciliation pass (squash-merged from claude/vs-port-4-10-3-polish)
