...
**Latest commit on `vs-port-fresh`:** `<TBD-4.2-port-squash>` (Phase 4.2-port squashed from `claude/vs-port-4-2-scout-index`).
...
```
<TBD>    [skip netlify] Phase 4.2-port: Scout Index + New Scout entry (squash-merged from claude/vs-port-4-2-scout-index)
