...
> **RLS standing rule (Phase 5.16.0).** HQ Core policies that were previously open-authenticated now gate on `(select public.is_active_member())` (non-pending, active user: admin / standard / freelance). Per-table "open to authenticated" notes below mean "active-member" as of 5.16.0. Canonical predicate definition, the `users_select` self-read carve-out, and full per-table posture live in `docs/auth-model.md` § RLS policies.
...
- RLS: see auth-model.md (SELECT active-member; admin INSERT/UPDATE/DELETE).
...
- Triggers: `updated_at_auto`, `activity_log_writer`.
- RLS: Freelance blocked entirely; SELECT + INSERT/UPDATE/DELETE for `admin` + `standard` only. Full posture in auth-model.md.
...
### vendors (renamed from `organizations` in Phase 5.2.3.B)

Holds the non-client rows from the unified `organizations` table (split into vendors + clients in 5.2.3; see v1-changelog). The rename preserved policies + indexes + OID-stable identifiers, so some underlying constraint/policy names still read as `clients_*`; column-level identifiers are post-5.2.3.

- `id` (uuid, PK, default `gen_random_uuid()`)
- `name` (text, NOT NULL)
- `category_id` (uuid, FK to `vendor_categories.id` ON DELETE SET NULL, nullable).
- `subcategory_id` (uuid, FK to `vendor_subcategories.id` ON DELETE SET NULL, nullable). Partial btree index `vendors_subcategory_idx WHERE subcategory_id IS NOT NULL`. App-side rule: when `category_id` changes, the UI clears `subcategory_id` since the prior pick may not belong to the new parent.
- `capabilities` (text[], default `{}`). Free-text vendor capability tags.
- `city` (text, nullable).
- `primary_address` (text, nullable).
- `website_url` (text, nullable).
- `general_email` (text, nullable). Company-level general email, distinct from the primary contact's `contact_email`. Surfaced on VendorDetail ("General Email") + VendorEdit ("General Email" in the Details section) + the bulk importer's `general_email` column.
- `tags` (text[], default `{}`).
- `preferred` (bool, NOT NULL, default false). Partial index `vendors_preferred_idx WHERE preferred = true`. Drives the Wiki "Preferred Vendors" embed.
- `nationwide` (bool, NOT NULL, default false). Marks vendors who work nationwide; the VendorsList city filter OR-s in any `nationwide = true` vendor so they surface under every city chip (client-side, via the `applyFilters` `fieldMatchAll` hook). No index (read only by the client-side list filter, never server-side). Editable via the VendorEdit "Nationwide" checkbox + the bulk importer's `nationwide` true/false column.
...
- `bulk_import_session_id` (uuid, FK to `bulk_import_sessions(id)` ON UPDATE CASCADE ON DELETE SET NULL, nullable). Stamped on every vendor the `bulk_import_commit_vendors` RPC creates or updates. Powers the "Bulk Imported" presence filter chip on `/vendors`. Partial index `vendors_bulk_import_session_idx (bulk_import_session_id) WHERE bulk_import_session_id IS NOT NULL`.
- `created_by` (uuid, FK to `users.id`)
- `created_at`, `updated_at` (timestamptz)
- Indexes: `vendors_city_idx (city) WHERE city IS NOT NULL`; `vendors_subcategory_idx`; `vendors_preferred_idx WHERE preferred = true`; `vendors_bulk_import_session_idx (bulk_import_session_id) WHERE bulk_import_session_id IS NOT NULL`.
- Triggers: `trg_activity_log_vendors` (AFTER INSERT/UPDATE/DELETE → activity_log via `activity_log_writer`); `trg_vendors_updated_at`.
- RLS: see auth-model.md (SELECT/INSERT/UPDATE active-member; DELETE admin-only). Underlying policy identifiers stay `clients_*` because the rename preserved policies by table OID.
...
- `vendor_files` (Phase 5.7.11): id (uuid, PK), vendor_id (FK ON DELETE CASCADE), title (text NOT NULL), url (text NOT NULL), created_by (FK to users **ON UPDATE CASCADE** ON DELETE SET NULL), created_at. Index `vendor_files_vendor_idx (vendor_id, created_at DESC)`. RLS: active-member SELECT/INSERT/DELETE; **no UPDATE policy (delete + re-add only)**. See auth-model.md. Any URL accepted (no format validation; producers paste Drive, Dropbox, Figma, vendor websites). Surfaced: VendorDetail Files & Assets card + VendorEdit Files & Assets section (gated on `!isCreate`).
- `venue_files` (Phase 6.4): mirror of `vendor_files`, vendor→venue. id (uuid, PK), venue_id (FK ON DELETE CASCADE), title (text NOT NULL), url (text NOT NULL), created_by (FK to users **ON UPDATE CASCADE** ON DELETE SET NULL), created_at. Index `venue_files_venue_idx (venue_id, created_at DESC)`. RLS: active-member SELECT/INSERT/DELETE; **no UPDATE policy (delete + re-add only)**. See auth-model.md. Any URL accepted. Surfaced: VenueDetail Files & Assets card (top of right aside) + VenueEdit Files & Assets section (gated on `!isCreate`).
- `vendor_ratings` (Phase 5.7.13): vendor_id (FK ON DELETE CASCADE), user_id (FK ON DELETE CASCADE), rating (int, CHECK BETWEEN 1 AND 5), created_at, updated_at. Composite PK `(vendor_id, user_id)`; UPSERT for re-rating. Indexes `vendor_ratings_vendor_idx` (vendor_id) + `vendor_ratings_user_id_idx` (user_id; the composite PK leads with `vendor_id`, so the `user_id` FK was otherwise unindexed). RLS: open-authenticated SELECT (aggregate needs cross-user reads); **self-only INSERT/UPDATE/DELETE via `user_id = auth.uid()`**. `trg_vendor_ratings_updated_at` trigger. Replaces the admin-curated `vendors.internal_rating` column dropped in Phase 5.7.13 (see changelog). Surfaced: VendorDetail Team Rating card (aggregate + viewer's own), VendorsList Rating column (team aggregate, rounded to half-star), Preferred Vendors wiki embed.
...
- RLS: see auth-model.md (SELECT/INSERT/UPDATE active-member; DELETE admin-only).
- Internal Notes: shared `notes_log` with `parent_type = 'client'`.

### organizations table (DROPPED)

Split into `vendors` (rename) + `clients` (new) in Phase 5.2.3; the `org_type` enum was dropped alongside. The current vendors / clients / projects / people / notes_log shapes below already reflect the split. See `docs/v1-changelog.md`; pre-split shape preserved in `OUTPUTS/historical/phase-5-2-2-spec.md`.

### projects
- `id` (uuid, PK)
- `name` (text, not null)
- `client_id` (uuid, FK to clients, nullable). The btree index follows the column name. `types.ts` is authoritative. (Round-tripped through an `organization_id` name during the 5.2.2 organizations rename and back in the 5.2.3 split; see changelog.)
- `status` (enum `project_status`, 14 values, default `Queued`): `Approved`, `In Production`, `In Progress`, `Location Scouting`, `Install`, `Removal`, `Billing`, `Queued`, `Quoting`, `Quote Sent`, `Awaiting Feedback`, `On Hold`, `Complete`, `Cancelled`. Reshaped to this locked list in Phase 5.2.1 (prior-value backfill map in the migration; see changelog).
- `job_number` (text, nullable). Surface 07 detail coral `#2604` job number. Partial btree index `projects_job_number_idx WHERE job_number IS NOT NULL`.
- `category` (text, nullable). "Pop-Up" / "Window Install" / "Trade Show" etc.
- `city` (text, nullable). Surface 04 List view column.
- `tags` (text[], default `{}`). Surface 07 detail "Summer 2026 / CPG / Outdoor" tag chips.
- `budget` (numeric, nullable). Surface 07 detail + Surface 08 edit. Planning reference figure, NOT an invoice amount; never renders on pipeline-summary surfaces per locked-decisions Q6 + spec § 5.A.7.
- `install_dates_start`, `install_dates_end` (date, nullable). Drives Calendar Install banners + Project Timeline Install bar + Project Detail "Days Until Install" countdown.
- `live_dates_start`, `live_dates_end` (date, nullable)
- `removal_dates_start`, `removal_dates_end` (date, nullable). Drives Calendar Removal banners + Project Timeline Removal bar.
- `production_folder_url`, `design_decks_folder_url`, `budget_sheet_url`, `slack_channel_url` (text, all nullable). Surfaced as the Production / Design / Budget / Slack quick-link buttons on ProjectDetail + editable in ProjectEdit. (`latest_creative_deck_url` dropped Phase 5.9.2.2, never surfaced in any UI; see changelog.)
- ~~`status_notes` / `client_notes` (text, nullable)~~ dropped in Phase 5.7.14; both moved to `notes_log` via `<InternalNotesEditor>` (status_notes content backfilled). See changelog.
- `archived_at` (timestamptz, nullable; null = active, non-null = archived; default queries filter `archived_at IS NULL`)
- `created_by` (uuid, FK to users)
- `created_at`, `updated_at`
- `bulk_import_session_id` (uuid, FK to `bulk_import_sessions(id)` ON UPDATE CASCADE ON DELETE SET NULL, nullable). Stamped on every project the `bulk_import_commit_projects` RPC creates or updates. Powers the "Bulk Imported" presence filter chip on `/projects`. Partial index `projects_bulk_import_session_idx (bulk_import_session_id) WHERE bulk_import_session_id IS NOT NULL`.
...
- `city` (text, nullable). Partial btree index `venues_city_idx WHERE city IS NOT NULL`.
- `venue_slide_url` (text, nullable). Google Slides URL surfaced on Surface 09 detail as a "Venue Slide" button next to the Edit button.
- `total_sq_ft` (int, nullable). Coexists with the shipped `square_footage` (total_sq_ft surfaced as "Total Sq Ft" in the wireframe).
- `exclusive_vendor_ids` (uuid[], NOT NULL default `{}`). Vendor references for the "Exclusive Vendors" link list on Surface 09 detail (renamed from `exclusive_vendors_org_ids` in the 5.2.3 split). Postgres cannot FK array elements; app validates entries reference valid vendors before write. Set only on the manual VenueEdit surface; the bulk importer deliberately does NOT touch this column (so a dedupe-update can't clobber curated values).
- `capacity`, `square_footage` (int)
- `website_url`, `contact_name`, `contact_email`, `contact_phone` (text)
- `general_email` (text, nullable). Company-level general email, distinct from any contact (mirrors `vendors.general_email`). Surfaced on VenueDetail ("General Email" row in Venue Details, hidden when empty) + VenueEdit ("General Email" in the Details section) + the bulk importer's `general_email` column, written through on both the RPC create + update paths.
- `features` (text[])
- `about_venue` (text). Free-form About Venue body on Surface 09 detail. Renamed from `notes` in Phase 5.10.0 (the `notes` name collided with the polymorphic `notes_log` Internal Notes table); OID-preserving RENAME COLUMN. The AI "Generate / Regenerate About Paragraph" button (VenueDetail + VenueEdit) writes this column via the `hq-generate-venue-about` edge function.
- `photos` (text[] NOT NULL DEFAULT '{}'; Supabase Storage paths in the `venue_photos` bucket). Convention: 4-element array with nulls for unpopulated slots, e.g. `ARRAY['<venue_id>/photo_1.jpg', NULL, '<venue_id>/photo_3.jpg', NULL]`. Path format: `<venue_id>/photo_<slot>.<ext>` where slot is 1-4. Populated by `vs-generate-deck` at deck-gen time via `persistVenuePhotosToHq` (runs after `pushVenuesToHq`, before Slides photo-load). Always overwritten with the producer's final `vs_venue_photos` state; prior HQ photos for the same slots are deleted first. Empty venues stay `'{}'` until a deck is generated involving them (no backfill per D5).
- `created_by` (uuid, FK)
- `created_at`, `updated_at`
- `bulk_import_session_id` (uuid, FK to `bulk_import_sessions(id)` ON UPDATE CASCADE ON DELETE SET NULL, nullable). Stamped on every venue the `bulk_import_commit_venues` RPC creates or updates. Powers the "Bulk Imported" presence filter chip on `/venues`. Partial index `venues_bulk_import_session_idx (bulk_import_session_id) WHERE bulk_import_session_id IS NOT NULL`.
...
- RLS: open-authenticated on SELECT/INSERT/UPDATE/DELETE (matches the shipped join-table convention). See auth-model.md.
...
- ~~`affiliations` (enum array `person_affiliation[]`)~~ dropped in Phase 5.2.3.C alongside the `person_affiliation` enum (locked Q4: at most one org type per person; FK presence resolves type). See changelog.
- `affiliation_type` (enum `person_affiliation_type`, NOT NULL, default `'Unaffiliated'`): `Client`, `Vendor`, `Venue`, `Unaffiliated`. Lets inline edit on PersonDetail clear an Organization FK without flipping the displayed type. Mutex CHECK `people_affiliation_type_mutex_check`: Client→vendor_id null, Vendor→client_id null, Venue/Unaffiliated→both FKs null. `personType()` helper in `src/lib/people/queries.ts` reads this column (with FK-derivation fallback for safety). PersonEdit writes `affiliation_type` explicitly on save.
- `client_id` (uuid, FK to clients, nullable, ON DELETE SET NULL) + `vendor_id` (uuid, FK to vendors, nullable, ON DELETE SET NULL). The org split of the former `organization_id` (5.2.3.C; see changelog). Mutex with `affiliation_type` per `people_affiliation_type_mutex_check`. Not every person ties to an org (Venue contacts may not).
- `venue_id` (uuid, FK to venues, nullable, ON DELETE SET NULL). Nullable per spec Q8 recommendation. (Legacy direct-link column; venue contacts now surface through the `venue_contact_people` join.)
- `role_title` (text), `email` (text), `phone` (text), `linkedin_url` (text)
- `tags` (text[], default `{}`)
- `created_by` (uuid, FK to users, NOT NULL, default ON DELETE RESTRICT)
- `created_at`, `updated_at`
- Indexes: `people_client_idx (client_id)` (renamed from `people_org_idx` in the 5.2.3 split), `people_vendor_idx (vendor_id)`, `people_full_name_idx (lower(full_name))`, `people_venue_idx (venue_id) WHERE venue_id IS NOT NULL`. (The `people_affiliation_gin_idx` GIN index was dropped with the `affiliations` column.)
- Triggers: `trg_people_updated_at`, `trg_activity_log_people` (AFTER INSERT/UPDATE/DELETE).
- RLS: see auth-model.md (active-member SELECT/INSERT/UPDATE/DELETE; HQ Core convention).
...
- `pool_summary` (text)
- `final_rankings` (jsonb): `[{candidate_id, final_rank, final_tier, rationale, recruiter_note, final_overview}]` where `final_overview` is a `string[]` of 4-6 short headlines per candidate. See `docs/decisions.md` § Talent Scout port (Phase 3).
...
- RLS: ALL gates on `is_active_member()` (see auth-model.md). Collaborative agency-wide workflow for active members; `pending` excluded. Applies to the three live VS tables (`vs_scouts`, `vs_candidate_venues`, `vs_venue_photos`).
...
- Brief fields inline (the Phase 4 brief-inline-on-vs_scouts decision (docs/decisions.md "Phase 4 cutover + port plan locked decisions")):
  ...
  - `budget` (numeric)
  - `brief_data` (jsonb, default `{}`): flexible per-scout extras the producer surfaces from the uploaded brief PDF. `toUpdate` drops keys whose form field is empty / empty-array / null. Form-backed intake keys are hoisted into dedicated form fields by `src/lib/venue-scout/briefForm.ts`. Live keys:
    - `expected_guest_count` (number; consumed by `vs-generate-deck` slide templating)
    - `notes` (string; retired key, no longer written by new scouts but preserved untouched on existing scouts; dumped verbatim into downstream research / compile prompts)
    - `uploaded_files` (string[]; storage paths under `briefs` bucket, append-only, for audit / re-parse)
    - `research_started_at` / `compile_started_at` / `deck_generation_started_at` (ISO timestamp strings; set by the respective edge fn at kickoff for idempotency, 90-second grace window per `docs/decisions.md`)
    - `overview_source_hash` (string; 16-char SHA-256 prefix over the 15 brief fields that drive the Event Overview prompt, written by `vs-generate-brief-overview` whenever it writes `event_overview`, read by Submit Brief in `BriefVenue.tsx` to decide whether the persisted overview is stale; machine metadata, no form field)
    - `install_dates` / `strike_dates` (string)
    - `activations_count` (number; slider, null = TBD so the key is dropped)
    - `objectives` (string[])
    - `target_audience` (string[]; tag-array shape, normalized from a legacy string shape in 5.12.5)
    - `vibe_aesthetic` (string[]; same 5.12.5 shape)
    - `target_neighborhoods` (string[]; sources values from the `neighborhoods` lookup parent-scoped to `vs_scouts.city` via `<RecordCombobox multi>` on BriefVenue + BriefReport; stored values are option-name strings, the lookup is the typeahead + curation source)
    - `strict_neighborhoods_only` (boolean; always written, false is meaningful)
    - `venue_types` (string[]; arbitrary strings, chip multi-select)
    - `sq_ft_min` / `sq_ft_max` / `sq_ft_minimum` (number; sliders, null = any so the key is dropped)
    - `ideal_features` (string[])
    - `priority_location` (`'high_foot_traffic' | 'intimate_destination'`)
    - `priority_cost` (`'lower_cost' | 'premium'`)

    `vs-research-venues` Phase B + `vs-generate-brief-overview` read these keys; downstream prompts stringify the entire jsonb so any key the producer adds gets seen by the AI.
...
Storage bucket: `vs_venue_photos` (private, signed URLs 1-hour TTL via `createSignedUrl(path, 3600)`). Storage RLS: single `FOR ALL TO authenticated` policy (USING true / WITH CHECK true), matching the `vs_*` table RLS; see auth-model.md for storage-bucket policies. Distinct from the public `venue_photos` bucket reserved for HQ Core's master `venues` table.
...
`ts-cron-monthly-spend-reset` (Phase 3.8) resets `anthropic_spend_current_month_usd` to 0 and `cap_alert_sent_this_month` to false on the 1st of each month at 00:01 UTC. Phase 5.15 added a 12-month prune of `anthropic_call_log` rows to the same cron (runs AFTER the global_settings reset so a prune failure cannot block the cap-alert re-arm).

### anthropic_call_log (Phase 5.15)

Append-only log of every successful Anthropic API call dispatched through `supabase/functions/_shared/anthropic.ts callClaude`. Drives the per-tool spend breakdown on HQ Admin Settings. Writes happen from the service-role wrapper; reads gate to admins only.

- `id` (uuid, PK, default `gen_random_uuid()`)
- `created_at` (timestamptz, NOT NULL, default `now()`)
- `app` (text, NOT NULL, CHECK `app IN ('talent_scout','venue_scout','hq')`): which per-app Anthropic key bucket the call ran under
- `fn_name` (text, NOT NULL): caller label from `CallClaudeOptions.fn_name` (e.g., `vs-research-venues`, `ts-pull-candidates`, `vs-research-venues:fit-rescue`, `findVenueWebsite`); falls back to `"unknown"` when omitted
- `model` (text, NOT NULL): the model id requested (defaults to `claude-sonnet-4-6`)
- `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_write_tokens` (int, NOT NULL, default 0): mirrored from the Anthropic response's `usage` block; cache columns map from `cache_read_input_tokens` / `cache_creation_input_tokens`
- `cost_usd` (numeric, NOT NULL, default 0): USD cost for this call as computed by `_shared/anthropic.ts calcCost` against the hardcoded `claude-sonnet-4-6` pricing constants
- `scout_id` (uuid, nullable, FK to `vs_scouts.id` ON DELETE SET NULL): set when the caller passed `scout_id` in `CallClaudeOptions`; null for callers with no scout in scope (TS callers, hq-generate-venue-about, pre-create flows). Delete-cascade-safe so a scout deletion doesn't drop the cost record.
- `role_id` (uuid, nullable, FK to `ts_roles.id` ON DELETE SET NULL): set when the caller passed `role_id`; null for VS callers and pre-create flows (ts-generate-scorecard, ts-refine-scorecard)
- Indexes:
  - `idx_anthropic_call_log_app_fn_created (app, fn_name, created_at desc)`: hot path for `anthropic_spend_breakdown` rollups + per-tool history scans
  - `idx_anthropic_call_log_created (created_at)`: drives the monthly cron's prune scan
  - `idx_anthropic_call_log_scout (scout_id) WHERE scout_id IS NOT NULL`: partial index for future per-scout drilldown
  - `idx_anthropic_call_log_role (role_id) WHERE role_id IS NOT NULL`: partial index for future per-role drilldown
- RLS: enabled. Single admin-only SELECT policy (`anthropic_call_log_admin_read`) that gates on `permission_role = 'admin'`. No INSERT/UPDATE/DELETE policies — writes flow from the service-role wrapper which bypasses RLS; 12-month prune runs from the monthly cron via the same service-role client.
- GRANTs: `SELECT` to `authenticated` (the policy filters); `ALL` to `service_role`.
- Lifecycle: appended on every successful `callClaude`. Failed calls (`!res.ok` early return) do NOT write a row, matching the `trackSpendAndAlert` posture. Pruned to the trailing 12 months by `ts-cron-monthly-spend-reset` on the 1st of each month.
...
- `promote_outlook_to_project(target_entry_id uuid) RETURNS uuid` (Phase 5.3): atomic promotion of an `outlook_entries` row to a `projects` row. INSERTs into `projects` with `name`, `client_id`, `city` from the entry + `status='Queued'` + `created_by=auth.uid()`; UPDATEs the entry's `linked_project_id` to the new project id; returns the new project id. Pre-checks `public.is_admin()` (raises 42501 on non-admin). Refuses to promote an already-linked entry (raises 23505). `SECURITY DEFINER` (cross-table writes don't trip RLS mid-flight). `GRANT EXECUTE TO authenticated`.

- `bulk_import_commit_projects(payload jsonb) RETURNS jsonb` (Phase 5.9.2): the atomic commit for the Projects bulk importer. One in-DB transaction owns the whole cross-table write: queued client/venue inserts (with a `_queued:N` id-translation table), novel `project_categories` / `cities` lookup auto-creates, per-row project insert (create path) or update (dedupe-update path), the `project_venues` join (the only import-owned roster), the `bulk_import_sessions` audit row, and one `bulk_import_session`/`bulk_import` `activity_log` row. **People roster (Account Lead / Designer / Team Members) is NOT imported** (5.9.2.1); it is set retroactively on the project edit page, and the dedupe-update path replaces only `project_venues` and never touches `project_account_managers` / `project_designers` / `project_members`. Per-project `activity_log` rows fire automatically via `trg_activity_log_projects`. Payload row keys mirror the template headers (date keys are the short form `live_start` etc.; the RPC maps them onto the DB `*_dates_*` columns); multi-value columns arrive as JSON arrays, `account_lead` as a single email string. `actor_id` comes from the payload (service-role invocation makes `auth.uid()` NULL inside the function); the body re-checks `permission_role='admin'` (raises 42501) as defense in depth. The final session UPDATE persists `imported_record_ids = v_created_ids` (the undo trail) and `imported_person_ids = '{}'` (projects create no People). `SECURITY DEFINER`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`. Helpers: `_bulk_import_resolve_client_ref(text, uuid[])` + `_bulk_import_resolve_venue_ref(text, uuid[])` (both `STABLE`, `GRANT EXECUTE TO authenticated`) resolve a ref string to an existing id, a `_queued:N` slot, or a fuzzy name match. The RPC exists because a PostgREST chain can't roll the cross-table write back atomically (see decisions.md).

- `bulk_import_commit_vendors(payload jsonb) RETURNS jsonb` (Phase 5.9.3): the atomic commit for the Vendors bulk importer. Same SECURITY DEFINER posture + admin re-check + `_queued:N` translation as `bulk_import_commit_projects`. One in-DB transaction owns: queued `vendor_categories` inserts, queued `vendor_subcategories` inserts (parent resolved per row), novel `vendor_capabilities` + `cities` lookup auto-creates, per-row vendor insert (create) or update (dedupe-update), the `bulk_import_sessions` audit row, and one `bulk_import_session`/`bulk_import` `activity_log` row. Deltas vs the Project RPC: `category` + `subcategory` are FK columns resolved via the two helpers (not free-text); `capabilities` is a `text[]` of NAMES written through with novel values auto-creating the `vendor_capabilities` lookup row; `preferred` + `nationwide` are each coerced from a `"true"`/`"false"`/`""` string (empty => false); `general_email` is written through as nullable text; **each row's `contact_name`/`email`/`phone` also creates (or vendor-scoped-dedupe-reuses) a `people` row affiliated to the vendor (`affiliation_type='Vendor'`, `vendor_id`, `created_by=actor`) so VendorDetail derives it as the Primary Contact; dedupe is scoped to the same vendor (not global) because the `people` affiliation mutex forbids relinking a person across orgs**; NO project-style staff-roster handling (vendors have no `vendor_account_managers` analog) and the dedupe-update path leaves `project_vendors` untouched. Subcategory parent resolution matches the queued subcategory's `parent_category` text against queued (this-batch) categories first, then existing, case-insensitive; unresolvable raises 23503. Queued `vendor_categories` insert uses `ON CONFLICT (lower(name))` (the unique arbiter is the `LOWER(name)` expression index, not a constraint) with a `SELECT id` fall-back so re-runs recover the existing id. Per-vendor `activity_log` rows fire automatically via `trg_activity_log_vendors`. `actor_id` comes from the payload; the body re-checks `permission_role='admin'` (raises 42501). The final session UPDATE persists `imported_record_ids = v_created_ids` and `imported_person_ids` = the freshly-INSERTED contact-person ids only (NOT the reuse branch) for the undo trail. `SECURITY DEFINER`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`. Helpers: `_bulk_import_resolve_vendor_category_ref(text, uuid[])` + `_bulk_import_resolve_vendor_subcategory_ref(text, uuid[], uuid)` (both `STABLE`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`).

- `bulk_import_commit_venues(payload jsonb) RETURNS jsonb` (Phase 5.9.4): the atomic commit for the Venues bulk importer. Same SECURITY DEFINER posture + admin re-check (42501) + `_queued:N` translation + single `bulk_import_sessions` row + one `bulk_import_session`/`bulk_import` `activity_log` row as `bulk_import_commit_vendors`. One in-DB transaction owns: queued `venue_types` creates, novel `cities` auto-creates (with `created_by` stamped), per-row venue insert (create) or update (dedupe-update on `lower(name)|lower(address)`), `venue_venue_types` join writes, and the per-row contact `people` row + `venue_contact_people` join row. Deltas vs the Vendor RPC: (a) **`venue_types` is a JOIN** (`venue_venue_types`), so resolution is two-step (queued create or case-insensitive existing match, then a `(venue_id, venue_type_id)` INSERT `ON CONFLICT DO NOTHING`); the dedupe-update path REPLACES the join (DELETE all rows for the venue, then re-INSERT). Queued `venue_types` use a `LOWER(name)=LOWER(...)` existence probe before INSERT (so admin-typed casing reuses an existing row) then `ON CONFLICT (name) DO NOTHING` (the arbiter is a real UNIQUE constraint, not an expression index); `venue_types` has NO `created_by` column so the INSERT omits it. (b) **`features` is a plain `text[]`** written through with no companion lookup / auto-create. (c) **`exclusive_vendor_ids` is deliberately NOT importable** (Jimmie's call 2026-05-20): the RPC never reads or writes the column, so a dedupe-update can't clobber a venue's manually-curated exclusive vendors. **Contact-People divergence (critical):** venues link a contact through the `venue_contact_people` JOIN table, NOT the dead legacy `people.venue_id` column. So each row's `contact_name`/`email`/`phone` creates (or venue-scoped-dedupe-reuses) a `people` row (`affiliation_type='Venue'`, `client_id`+`vendor_id` NULL per the affiliation mutex, `venue_id` left NULL) AND inserts a `venue_contact_people (venue_id, person_id)` row `ON CONFLICT DO NOTHING`; that join row is what surfaces the contact on VenueDetail. Venue-scoped dedupe matches existing contacts THROUGH the join, NAME first then email (locked 2026-05-20). The join is ADDITIVE on the update path (never replaced), in contrast to `venue_venue_types`. `general_email` is written through as nullable text on both paths. After the contact-People block, the row's `event_day_rate` (defensively stripped of `$`/commas) appends one `venue_rate_history` `event_day` row dated `current_date` ONLY when it differs (`IS DISTINCT FROM`) from the venue's current most-recent `event_day` amount; a repeat import with an unchanged rate is a no-op (tracked by the `created_refs.venue_rates` counter). The final session UPDATE persists `imported_record_ids = v_created_ids` and `imported_person_ids` = the freshly-INSERTED contact-person ids only (NOT the reuse branch) for the undo trail. Per-venue `activity_log` rows fire via `trg_activity_log_venues`; per-people rows via `trg_activity_log_people` (only on newly-created contacts). `actor_id` comes from the payload; the body re-checks `permission_role='admin'` (raises 42501). `SECURITY DEFINER`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`. Helper: `_bulk_import_resolve_venue_type_ref(text, uuid[])` (`STABLE`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`).

- `bulk_import_undo(p_session_id uuid, p_actor_id uuid, p_dry_run boolean DEFAULT false) RETURNS jsonb` (Phase 5.9.6): reverts a committed bulk import. Called from the `bulk-import` edge function's `undo` mode. Re-checks `p_actor_id` is an admin (raises 42501), the third gate after AdminRoute + the edge function's server-side re-check; `actor_id` comes from the caller because the service-role invocation makes `auth.uid()` NULL. Gates: session must exist (P0002), `status='committed'` (else 22023), `committed_at >= now() - 7 days` (`v_window_days constant := 7`, keep in sync with the client `BULK_IMPORT_UNDO_WINDOW_DAYS`; else 22023), and `imported_record_ids` non-empty (else 22023 "no undo trail", i.e. pre-5.9.6 untracked sessions). Builds per-entity cascade counts (`deliverables`/`tasks`/`project_*` joins for projects; `vendor_files`/`vendor_ratings`/`project_vendors` for vendors; `venue_venue_types`/`venue_contact_people` for venues). When `p_dry_run`, returns `{ ok, dry_run:true, counts }` WITHOUT writing. Otherwise: DELETEs the tracked `imported_person_ids` from `people` first (the org FK is ON DELETE SET NULL, so they would orphan), then DELETEs the `imported_record_ids` from the entity table (cascades remove deliverables/tasks/joins/ratings/files), DELETEs the original `bulk_import`/`bulk_import_session` activity row, INSERTs one `bulk_import_undo`/`bulk_import_session` activity row with the counts payload (`entity_id` keeps the now-deleted session id; not an FK), and hard-deletes the `bulk_import_sessions` row last. **Never touches shared lookups (`cities`, `vendor_categories`/`vendor_subcategories`/`vendor_capabilities`, `venue_types`) or queued clients/venues a project import created (§14-A)**, only the records + their import-created contacts. Reused/pre-existing contacts survive (they were never tracked). Counts shape: `{ entity_type, records, contacts, cascade:{...} }`. Returns `{ ok, dry_run, counts }`. `SECURITY DEFINER`, `SET search_path = public`, `GRANT EXECUTE TO authenticated`. DESTRUCTIVE (gated).

- `vs_research_try_acquire_kickoff(target_scout_id uuid, grace_seconds int DEFAULT 360) RETURNS boolean` (Phase 5.12.1): atomic kickoff acquisition for `vs-research-venues` (replaces a TOCTOU inline check-then-write that double-spent tokens live; code-observations Edge #1, see decisions.md). Uses `pg_try_advisory_xact_lock` over a deterministic 64-bit key derived from `hashtextextended('vs-research:' || target_scout_id::text, 0)`; if another transaction holds the lock, returns false immediately. Otherwise reads `brief_data->>'research_started_at'` and bails (returns false) when the prior kickoff is within `grace_seconds`. On success: updates the scout's `brief_data.research_started_at` to `now()` and clears `pipeline_error` in the same transaction, then returns true. Lock auto-releases at transaction end. `SECURITY DEFINER`, `SET search_path = public`. **GRANTs locked down:** REVOKE EXECUTE FROM PUBLIC + anon + authenticated, GRANT EXECUTE TO `service_role` only (edge-only caller via the service-role client; the RPC performs no caller-ownership check, so an authenticated grant would let any user reset another scout's kickoff state). See `docs/conventions.md` § Schema migrations for the service-role-vs-invoker decision rule.

- `vs_deck_try_acquire_kickoff(target_scout_id uuid, grace_seconds int DEFAULT 90) RETURNS boolean` (Phase 5.12.4.1): atomic kickoff acquisition for `vs-generate-deck` (replaces a TOCTOU inline check-then-write of `brief_data.deck_generation_started_at` where two near-simultaneous invocations could both call `pushVenuesToHq` and INSERT duplicate `venues` rows; see decisions.md). Mirrors `vs_research_try_acquire_kickoff`: `pg_try_advisory_xact_lock` over a deterministic 64-bit key from `hashtextextended('vs-deck:' || target_scout_id::text, 0)` (a separate `vs-deck:*` namespace from `vs-research:*` so deck + research kickoffs for the same scout don't collide on the advisory lock space); if another transaction holds the lock, returns false immediately. Otherwise reads `brief_data->>'deck_generation_started_at'` and bails (returns false) when the prior kickoff is within `grace_seconds`. On success: updates `brief_data.deck_generation_started_at` to `now()` and clears `pipeline_error` in the same transaction, then returns true. Lock auto-releases at transaction end. `SECURITY DEFINER`, `SET search_path = public`. **GRANTs locked down:** REVOKE EXECUTE FROM PUBLIC + anon + authenticated, GRANT EXECUTE TO `service_role` only (same edge-only / no-caller-ownership-check rationale as `vs_research_try_acquire_kickoff`).

- `anthropic_spend_breakdown(window_kind text DEFAULT 'month', window_iso text DEFAULT NULL) RETURNS TABLE (app text, fn_name text, calls bigint, total_cost_usd numeric, avg_cost_usd numeric)` (Phase 5.15): per-tool spend rollup over `public.anthropic_call_log` for the selected window. `window_kind` is `'month'` (default) or `'year'`; the function `date_trunc(window_kind, anchor)`s the anchor into a window start + adds the matching interval for the exclusive end. `window_iso` accepts an ISO date or timestamp; NULL (default) means `now()`. Other `window_kind` values raise `'anthropic_spend_breakdown: window_kind must be ''month'' or ''year''...'`. Returns one row per `(app, fn_name)` with call count + total + per-call average, ordered by `total_cost_usd DESC`. **Admin gate inside the body** via `public.is_admin()` (raises `'anthropic_spend_breakdown: admin only'` for non-admins). `SECURITY DEFINER`, `STABLE`, `SET search_path = public`. `REVOKE EXECUTE FROM PUBLIC`, `GRANT EXECUTE TO authenticated` (the inline gate makes the authenticated grant safe). Consumed by `AnthropicSpendBreakdownTable.tsx` (HQ Admin Settings) and by the TS + VS Settings read-only cards (which sum rows for their own app client-side to derive per-app + global spend for the toggle-selected window).

- `increment_anthropic_spend(p_cost numeric) RETURNS TABLE (before_usd numeric, after_usd numeric, cap_usd numeric, just_crossed boolean)` (Phase 6.0 F004): atomic spend-counter increment for `_shared/anthropic.ts trackSpendAndAlert` (replaces a non-atomic SELECT-then-UPDATE that lost spend and mis-fired the cap-alert under concurrent `callClaude`; see decisions.md). Reads the single `global_settings` row `FOR UPDATE` (serializes concurrent callers under READ COMMITTED, so each reads the prior commit's value), adds `p_cost` to `anthropic_spend_current_month_usd`, and sets `cap_alert_sent_this_month` iff this call crosses the cap, returning before/after/cap/just_crossed so exactly one caller emails. `SECURITY DEFINER`, `SET search_path = public`. **GRANTs locked down:** REVOKE EXECUTE FROM PUBLIC + anon + authenticated, GRANT EXECUTE TO `service_role` only (edge-only caller; no caller-ownership check; same posture as `vs_research_try_acquire_kickoff`).

- `reset_scout_for_deck_regenerate(target_scout_id uuid) RETURNS void` (Phase 4.10.6-port): atomic state-reset RPC for the Deck Prep regenerate flow. Called from DeckPrep.tsx `generate()` when a producer clicks Generate Deck on a scout that already has a successful prior deck. Sets `current_step='deck_prep'` + strips `deck_generation_started_at` from `brief_data` via the `jsonb -` operator + clears `status='in_progress'` + `pipeline_error=null` + bumps `last_touched_at`. Single SQL statement so there's no TOCTOU race between read and write (which the prior frontend read-modify-write had). Idempotent: the `brief_data -` is a no-op for the missing key. `SECURITY INVOKER`. `GRANT EXECUTE TO authenticated` (browser caller, relies on RLS for caller scope; contrast the service-role-only kickoff locks above).
