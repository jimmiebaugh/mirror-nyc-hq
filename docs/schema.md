...
- `photos` (text[] NOT NULL DEFAULT '{}'; Supabase Storage paths in the `venue_photos` bucket). Convention: 4-element array with nulls for unpopulated slots, e.g. `ARRAY['<venue_id>/photo_1.jpg', NULL, '<venue_id>/photo_3.jpg', NULL]`. Path format: `<venue_id>/photo_<slot>.<ext>` where slot is 1-4. Phase 5.14: populated by `vs-generate-deck` at deck-gen time via `persistVenuePhotosToHq` (runs after `pushVenuesToHq`, before Slides photo-load). Always overwritten with the producer's final `vs_venue_photos` state; prior HQ photos for the same slots are deleted first. Empty venues stay `'{}'` until a deck is generated involving them (no backfill per D5).
- `created_by` (uuid, FK)
- `created_at`, `updated_at`
...
- `anthropic_spend_breakdown(window_kind text DEFAULT 'month', window_iso text DEFAULT NULL) RETURNS TABLE (app text, fn_name text, calls bigint, total_cost_usd numeric, avg_cost_usd numeric)` (Phase 5.15, migrations `20260608000000_phase_5_15_anthropic_call_log.sql` + `20260609000000_phase_5_15_3_spend_breakdown_window.sql`): per-tool spend rollup over `public.anthropic_call_log` for the selected window. `window_kind` is `'month'` (default) or `'year'`; the function `date_trunc(window_kind, anchor)`s the anchor into a window start + adds the matching interval for the exclusive end. `window_iso` accepts an ISO date or timestamp; NULL (default) means `now()`. Other `window_kind` values raise `'anthropic_spend_breakdown: window_kind must be ''month'' or ''year''...'`. Returns one row per `(app, fn_name)` with call count + total + per-call average, ordered by `total_cost_usd DESC`. **Admin gate inside the body** via the canonical `public.is_admin()` helper (raises `'anthropic_spend_breakdown: admin only'` for non-admins). `SECURITY DEFINER`, `STABLE`, `SET search_path = public`. `REVOKE EXECUTE FROM PUBLIC`, `GRANT EXECUTE TO authenticated` (the inline gate makes the authenticated grant safe — non-admins surface the exception instead of data). Consumed by `AnthropicSpendBreakdownTable.tsx` (HQ Admin Settings) and by the TS + VS Settings read-only cards (which sum rows for their own app client-side to derive per-app spend + global spend for the toggle-selected window). The second migration explicitly `DROP FUNCTION IF EXISTS public.anthropic_spend_breakdown(text)` before re-creating with the two-arg signature, because `CREATE OR REPLACE` on a different signature creates a sibling function and PostgREST resolution would be ambiguous.
