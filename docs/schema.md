...
- `research_error` (text, nullable, Phase 4.5-port): persisted error message from the most recent `vs-research-venues` run. NULL when no failure on the latest run. The Researching page Realtime-subscribes to `vs_scouts` and on non-null `research_error` with `status='failed'`, navigates to `/sourcing/error/research-timeout`. `vs-research-venues` clears it at kickoff so a retry from a prior failure starts clean.
