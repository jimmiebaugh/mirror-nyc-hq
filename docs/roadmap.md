...
- **5.10 Venue About-paragraph generator + `about_venue` rename. ACTIVE.**
  Standalone follow-on after the Phase 5.9 close, re-opening venue-side
  work. Numbering note (2026-05-21): this phase took the 5.10 slot
  previously sketched for the reviews; TS review / VS review / mobile
  bumped to 5.11 / 5.12 / 5.13.
  - **5.10.0 `venues.about_venue` rename + AI About generator.** Two
    related additions in one squash, one Netlify deploy. (A) `RENAME`
    `venues.notes` -> `venues.about_venue` (OID-preserving; the `notes`
    name collided with the polymorphic `notes_log` Internal Notes table)
    plus the `CREATE OR REPLACE bulk_import_commit_venues` rebased onto
    the 5.9.7 body with the three column references swapped, and the
    importer payload + CSV template header changed in lockstep. (B) New
    `hq-generate-venue-about` edge function (first `callClaude('hq', ...)`
    consumer; `OVERVIEW_TOOL` + `OVERVIEW_SYSTEM` lifted verbatim to
    `_shared/venueOverview.ts`; web_search included, `tool_choice: auto`)
    + a Generate / Regenerate About Paragraph button on BOTH VenueDetail
    (card header) and VenueEdit (About Venue section header; disabled
    while the form is dirty). Regenerate gates behind an AlertDialog;
    Generate goes straight to the call. Breaking-rename coordination: the
    migration + function deploy + frontend ship together as one
    Jimmie-approved push. **Status:** DONE (`acf6d4b`). Spec:
    `OUTPUTS/phase-5-10-0-spec.md`.
  - **5.10.1 Venue Edit + Detail layout refresh.** Frontend-only UX pass
    (its own single Netlify deploy). VenueEdit Details card rebuilt as a
    2-column, label-above layout with `card-headbar` headers; standalone
    Venue Types card folded in (colored pills); "Links & References" became
    the "Master Venue Deck Slide" card (Venue Slide button + edit toggle);
    Features is now a tag input; section order Details -> Master Venue Deck
    Slide -> About Venue -> Rates (2-col) -> Exclusive Vendors. VenueDetail
    restyled to match (label-above `DField`, editable Name field + static
    page title, Rates + Exclusive Vendors as their own cards, wider 3-col
    address, paired-field alignment). First responsive `@media` breakpoints
    (form pairs collapse under 640px); `.label-form` coral -> grey app-wide.
    No-spec, notes-driven; iterated live. **Status:** DONE (`e8f8cdc`).
- **5.11 Talent Scout review.** Jimmie's notes + active bug hunting
  ...
  when batch arrives.
- **5.12 Venue Scout review.** Jimmie's notes + active bug hunting +
  ...
  arrives.
  - **Overview standardization carry-forward (locked 2026-05-21, deferred
    from 5.10.0).** Goal: the "About Venue" / venue overview paragraph is
    ONE evergreen artifact, generated by the SAME shared prompt
    (`_shared/venueOverview.ts` `OVERVIEW_SYSTEM` + `OVERVIEW_TOOL`,
    rewritten evergreen in 5.10.0) in both HQ and VS, stored in
    `venues.about_venue`, and reusable across future decks. 5.10.0 locked
    the shared evergreen prompt + the HQ generator; the VS-side rework
    below was deferred here so it can be tested as one coherent change.
    **Full VS workflow (as of 2026-05-21):** brief upload (optional) →
    brief parsed for client/event info + venue parameters entered → event
    overview generated → venue-research sheet upload (optional, parsed) →
    venue research (event+venue brief + target city/neighborhood passed to
    Claude to research venues matching the params; count depends on how
    many the user uploaded) → sourcing table lists all venues (uploaded +
    Claude-sourced) with Claude-researched context (address, website,
    features, recommendations, considerations, etc.) → user shortlists +
    adds notes → final review (user confirms/edits all venue info, uploads
    up to 4 photos/venue, notes resurfaced + editable) → deck-preview
    compile → Generate Deck. **Changes to make here:**
    (1) **Move overview generation to the deck-preview-compile stage** (it
    currently runs in `vs-compile-summaries` Pass 2). At that stage pass
    name, size, features, Claude's recommendations + considerations,
    website, and user notes/feedback through the SHARED locked prompt —
    inputs are then ~the same as the HQ function (VS just carries the extra
    recs/considerations/notes). Recs/considerations are generated EARLIER
    (at the research step, surfaced in the sourcing table + final review),
    NOT at overview time.
    (2) **Add `web_search` to the VS overview call** (Jimmie's call
    2026-05-21). This requires switching that call from forced
    `tool_choice: { type: tool }` to `tool_choice: auto` (a forced custom
    tool short-circuits the server-side search), adding the same
    tool_use → text → stub fallback the HQ function uses, and re-tuning the
    180s `WORK_TIMEOUT_MS` (web_search × N venues in the sequential batch
    loop, on top of Pass 1's search, can blow the ceiling). Needs a real
    VS sourcing → compile → deck smoke test.
    (3) **Move the VS → HQ venue DB push to the "Generate Deck" click**
    (currently the `vs_candidate_venues_shortlist_sync` trigger upserts a
    `venues` row at SHORTLIST time, carrying only name/address/
    neighborhood/website/features — and NOT the overview). Rework so the
    push happens on Generate Deck (the user may edit the About paragraph
    one last time before that), links ALL relevant VS fields to their HQ
    `venues` columns, AND saves the generated paragraph into
    `venues.about_venue`. We don't need a venue in the HQ DB until it's
    actually included in a deck. This closes the sync gap: today the
    VS-generated overview never reaches `venues.about_venue`.
    (4) **When VS converges to the shared overview prompt, go tool-less +
    add a prompt-cache breakpoint.** 5.10.0 made HQ tool-less (plain text +
    web_search, no `write_overview` tool — removes the collapse-risk class);
    VS Pass 2 still uses the forced `write_overview` tool, so converging it
    means switching to the same tool-less plain-text path. Because VS
    generates N venues per scout in a sequential loop, add a prompt-cache
    breakpoint on the shared system prompt (the `callClaude` wrapper supports
    `cache_control` on a `system` content-part array + the
    `prompt-caching-2024-07-31` beta) so the long evergreen system prompt +
    few-shot examples are billed once per scout, not once per venue. (HQ is
    on-demand single calls, so caching is irrelevant there — VS-batch only.)
    (5) **Update the `write_overview` tool when VS converges OR retire it.**
    If VS also goes tool-less, the `write_overview` tool in
    `_shared/venueOverview.ts` can be deleted entirely; otherwise apply the
    deferred edits (drop "tied to the brief", cut the "critical
    consideration" invitation, scrub forbidden words "premier"/"elevated"
    from the examples, retarget to 3-5 sentences / ~80-110 words).
- **5.13 Mobile styling pass.** Cross-cutting responsive sweep
