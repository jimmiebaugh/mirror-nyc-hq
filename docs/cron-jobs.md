...
### `ts-cron-pull-watchdog` (every 5 min)
Detects stalled `ts_pull_rounds` and flips `status='running'` rows whose `updated_at` is older than 60 minutes to `status='stalled'`. The chunked pipeline writes `processed_count + pending_candidates` on every batch boundary, so `updated_at` is the reliable heartbeat. Detect-and-flag only — surfaces in PullDetail UI for manual retry. 60 minutes covers large pools where a single Anthropic batch with big resume PDFs can sit for a while.
