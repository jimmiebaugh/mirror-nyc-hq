...
Producer-driven iteration to restore live URL fetching + venue research quality after Anthropic environment changes invalidated the prior `claude-sonnet-4-5 + web_search_20250305` baseline. Pivoted vs-research-venues Phase A + Phase B + vs-compile-summaries Pass 1 to `claude-sonnet-4-6 + web_search_20250305` (the new model is on web_search's supported list; the simpler 20250305 tool keeps per-turn token bloat bounded vs the dynamic-filter `20260209` version). Added `pause_turn` continuation loop to `_shared/anthropic.ts callClaude` (cap 1 cycle) so server-tool-heavy calls return final tool_use blocks instead of failing "no structured output." `writeFailure` CAS guards (`.eq("current_step", <expected_pre_success_step>)`) on all three AI edge functions prevent failure-overwrites-success when parallel invocations race. Sized timeouts for Supabase Pro plan: WORK_TIMEOUT_MS = 360s with 40s buffer under 400s platform wall clock. Trimmed prompt context: removed full `brief_data` JSON dump from all three Claude user messages, extracting only `expected_guest_count` + `brief_data.notes` selectively. Added `isPlaceholderString` + `stripPlaceholders` to `_shared/venueTypes.ts` so `<UNKNOWN>` / `TBD` / `N/A` tokens never reach the DB. FILL_TOOL schema: added `key_features` to required; placeholder warnings in tool descriptions + user-message trailer. Dropped forced `tool_choice` on Phase A enrichSheetVenue + Pass 1 fill (mirrors Phase B's earlier auto pivot; under heavy schema descriptions + web_search, forced tool_choice was causing minimum-output collapse). Researching loading copy updated to "could take up to 4 minutes."

**End-user win:** sourcing + enrichment work end-to-end again. server_tool_uses > 0 on every Claude call; per-call costs bounded; placeholder strings never reach the matrix.
...
- **Cutover sequence (locked Phase 4.10.3-port):**
  1. **Identify must-carry main commits.** `vs-port-fresh` branched from `dd38577`. Everything on main after that point falls into two buckets: (a) the archived failed-Phase-4 stack + URL-quality hot patches (we DISCARD these on cutover -- they're the failed attempt) and (b) parallel work that landed on main during the port window and needs to come along (currently just `f24d3f5`: 2026-05-13 TS Final Review packet + email template fixes). Anything in bucket (b) is the must-carry set.
  2. **Cherry-pick the must-carry set** onto `vs-port-fresh`: `git cherry-pick f24d3f5` (plus any future TS-side commits if more land between now and cutover). Each cherry-pick creates a new SHA on `vs-port-fresh` -- that's expected. No conflicts expected since TS files are disjoint from VS port files. Verify clean apply by checking the new commit's diff matches the original.
  3. **Hard-reset main to `vs-port-fresh` HEAD.** `git push origin vs-port-fresh:main --force-with-lease`. This is the cutover event; fires the Netlify deploy. The archived failed-Phase-4 stack on main is intentionally discarded -- the new main starts with the port + cherry-picked TS work. Alternative if you'd rather preserve the failed-attempt history in `main`'s log: squash-merge as one big "Phase 4: Venue Scout port" commit; cherry-pick step still applies first.
  4. **NOTE on SHA-based superset check:** `git log main --not vs-port-fresh --oneline` will NOT be empty by design (main has the archived stack; vs-port-fresh has cherry-picks with new SHAs). Don't use that as a gate. The real check is: "did the cherry-pick(s) apply without conflict + does the resulting commit's diff equal the source commit's diff?" `git show --stat <new-sha>` vs `git show --stat <source-sha>` is the readable comparison.
