...
- `vs-research-venues({ scout_id })` (Phase 4.5-port rebuild; Phase 4.10.3-port adds Phase A sheet-row enrichment + HEAD-validates URLs + tightens Recs/Considerations schema + tightens `website_url` schema + pivots to claude-sonnet-4-5): AI venue work in two parallel phases inside one `EdgeRuntime.waitUntil`. **Phase A (4.10.3-port consolidation):** SELECT `vs_candidate_venues WHERE scout_id=? AND source='sheet'`. For each row, `callClaude('venue_scout', { tool: fill_venue, web_search: max_uses=2 })`. Per-row patch guards prevent overwriting producer values. Per-row failures tolerated (vs-compile-summaries Pass 1 is the backstop). **Phase B (existing):** AI-driven new-venue research via `callClaude('venue_scout', ...)` with the `submit_research` custom tool + `web_search_20250305` server tool. Returns `{ ok, scout_id }` immediately; AI work runs in `EdgeRuntime.waitUntil`. On completion, INSERTs canonicalized + HEAD-validated venues into `vs_candidate_venues` (every `website_url` runs through `_shared/urlValidation.validateWebsiteUrls`: sanitizer + parallel HEAD-request gate against URL fabrication; rejects 4xx, host-mismatch redirects, and shorter-path soft-404 redirects; 5xx and network errors keep the URL so transient failures don't strip working venues) with `source='research'`, then writes `derived_columns` + `current_step='sourcing_report'` + `status='in_progress'` + `pipeline_error=null` to `vs_scouts`. On any failure (Claude error, no tool output, empty sanitized result, insert error, 120s timeout), writes `status='failed'` + `pipeline_error=<message>`. Idempotent on page refresh via `brief_data.research_started_at` 90-second grace window. Model: `claude-sonnet-4-6` (wrapper default; testing 4-6 on the port-side function, pivot to 4-5 per `docs/decisions.md` Phase 4.5-port if `web_search` regresses). User-invoked, `verify_jwt = true`. The Researching page Realtime-subscribes to `vs_scouts` for completion / failure. Does NOT slot-replace the failed-attempt `vs-start-sourcing` (different function name); that still queues for cutover deletion.
- `vs-compile-summaries({ scout_id })` (Phase 4.7.2-port rebuild + Phase 4.10.1-port Pass 1 extension): two-pass venue compilation via `callClaude('venue_scout', ...)`. Pass 1 = `fill_venue` tool (originally manual venues with missing fields only; populates `venue_type` / `website_url` / `size_sq_ft` / `capacity` / `key_features` / `derived_attrs` / `recommendations` / `considerations` / `rank` from training-knowledge + brief + producer notes, with `canonicalizeType` + `validateWebsiteUrl` (Phase 4.10.3-port HEAD-validation wrapper around `sanitizeWebsiteUrl`) post-emission). **Phase 4.10.1-port:** `FILL_TOOL` + `FILL_SYSTEM` + `buildFillUserMsg` extracted to `_shared/venueFill.ts` (single source of truth shared with vs-parse-sheet). Pass 1 `needsFill` condition extended to also fire for `source='sheet'` rows missing `derived_attrs` (vs-parse-sheet enrichment never fills derived_attrs because `vs_scouts.derived_columns` doesn't exist at parse time). Per-field patch guards already prevent overwriting recs / rank / type / etc. that vs-parse-sheet filled. Pass 2 = `write_overview` tool (every pitched venue; regenerated on every compile; producer can manually edit on Review). Returns `{ ok, scout_id }` immediately; AI work runs in `EdgeRuntime.waitUntil`. On completion, writes `current_step='deck_prep'` + `status='in_progress'` + clears `pipeline_error`. On catastrophic failure (DB connection, 180s timeout, final UPDATE error), writes `status='failed'` + `pipeline_error=<msg>`. Per-venue Claude errors logged + tolerated (matches VS Pro). Idempotency via `brief_data.compile_started_at` (90-second grace, parallel to 4.5-port). Sequential loop; 180-second `Promise.race` ceiling (vs 4.5-port's 120) to accommodate per-venue arithmetic. Model: `claude-sonnet-4-6` (wrapper default; independent test from 4.5-port; pivot to 4-5 per inline procedure if collapse signature recurs). No `web_search` (relies on training knowledge + brief context). User-invoked, `verify_jwt = true`. The Compiling page Realtime-subscribes to `vs_scouts` for completion / failure. Replaces the failed-attempt slot in production, parallel to vs-parse-brief and vs-parse-sheet pattern.
- `vs-generate-deck({ scout_id })` (Phase 4.8.2-port rebuild; slide indices corrected in 4.8.3-port): Google Slides deck generation. Authenticates via `_shared/googleServiceAccount.getGoogleAccessToken([presentations, drive])` (no impersonation; service account owns the Drive + Slides API calls and is a member of the Mirror Shared Drive). Copies `GOOGLE_TEMPLATE_FILE_ID` into `GOOGLE_OUTPUT_FOLDER_ID` as `${event_name} - Venue Pitch Deck v${N}` (hyphen, not em dash, per voice rule). Mirror's template has 6 front-matter slides (cover, project info across slides 2-3, event overview, section title, venue map with venue-name legend at slide 6). Slide 7 = per-venue detail template (duplicated per venue, original deleted). Slide 8 = per-venue floor plan template (duplicated per venue, original deleted). Function references `legendSlideId = slides[5]`, `templateSlide7 = slides[6]`, `templateSlide8 = slides[7]`. Populates front-matter global tokens (`client_name`, `event_name`, `event_live_date`, `guest_count`, `event_location`, `event_overview`) across the front matter, and legend tokens (`venue_1_name` through `venue_7_name`) scoped to `legendSlideId`. Per-venue scoped text replacements on the duplicated detail slide (`venue_name` ALL-CAPS, `venue_id_padded`, `venue_address`, `venue_neighborhood`, `venue_overview`, `venue_size`, `venue_capacity`, `venue_features`, `venue_website`); `venue_name` also applied to the duplicated floor plan slide so both share the ALL-CAPS header. "Website" hyperlinking on the detail slide (best-effort; tolerated on failure). Image replacement by alt text (`img_1` through `img_4`) on the detail slide using signed URLs (`createSignedUrl(path, 3600)` against the private `vs_venue_photos` bucket). Deletes the original template slides 7 + 8. Returns `{ ok, scout_id }` immediately; AI-shaped work runs in `EdgeRuntime.waitUntil`. On success, appends metadata to `vs_scouts.generated_decks` jsonb (`{ deck_id, deck_name, version, generated_at, venue_count, slide_count, edit_url, embed_url }`) + flips `current_step='completed'` + `status='complete'` + clears `pipeline_error`. On failure, writes `status='failed'` + `pipeline_error=<CODE>: <message>` (current_step stays at `deck_prep` so the producer can re-trigger from DeckPrep). Error codes: `AUTH_FAILED`, `TEMPLATE_COPY_FAILED`, `SLIDES_API_FAILED`, `NO_VENUES_INCLUDED`, `UNKNOWN`. Idempotency via `brief_data.deck_generation_started_at` (90-second grace, parallel to 4.5 / 4.7.2). 180-second `Promise.race` ceiling. User-invoked, `verify_jwt = true`. The Generating page Realtime-subscribes to `vs_scouts` for completion / failure (parses the code from `pipeline_error` with `^([A-Z_]+):` regex and routes to `/deck/error/<code>`). Slot-replaces the failed-attempt `vs-generate-deck` in production.
