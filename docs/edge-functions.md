...
- `vs-generate-deck({ scout_id })` (Phase 4.8.2-port rebuild): Google Slides deck generation. Authenticates via `_shared/googleServiceAccount.getGoogleAccessToken([presentations, drive])` (no impersonation; service account owns the Drive + Slides API calls and is a member of the Mirror Shared Drive). Copies `GOOGLE_TEMPLATE_FILE_ID` into `GOOGLE_OUTPUT_FOLDER_ID` as `${event_name} - Venue Pitch Deck v${N}` (hyphen, not em dash, per voice rule). Populates slide 2/3/5 global tokens (`client_name`, `event_name`, `event_live_date`, `guest_count`, `event_location`, `event_overview`, `venue_1_name` through `venue_7_name`). Duplicates template slide 6 (venue detail) + slide 7 (floor plan) once per pitched + `include_in_deck=true` venue. Per-venue scoped text replacements (`venue_name`, `venue_id_padded`, `venue_address`, `venue_neighborhood`, `venue_overview`, `venue_size`, `venue_capacity`, `venue_features`, `venue_website`), "Website" hyperlinking on slide 6 (best-effort; tolerated on failure), and image replacement by alt text (`img_1` through `img_4`) using signed URLs (`createSignedUrl(path, 3600)` against the private `vs_venue_photos` bucket). Deletes the original template slides 6 + 7. Returns `{ ok, scout_id }` immediately; AI-shaped work runs in `EdgeRuntime.waitUntil`. On success, appends metadata to `vs_scouts.generated_decks` jsonb (`{ deck_id, deck_name, version, generated_at, venue_count, slide_count, edit_url, embed_url }`) + flips `current_step='completed'` + `status='complete'` + clears `research_error`. On failure, writes `status='failed'` + `research_error=<CODE>: <message>` (current_step stays at `deck_prep` so the producer can re-trigger from DeckPrep). Error codes: `AUTH_FAILED`, `TEMPLATE_COPY_FAILED`, `SLIDES_API_FAILED`, `NO_VENUES_INCLUDED`, `UNKNOWN`. Idempotency via `brief_data.deck_generation_started_at` (90-second grace, parallel to 4.5 / 4.7.2). 180-second `Promise.race` ceiling. User-invoked, `verify_jwt = true`. The Generating page Realtime-subscribes to `vs_scouts` for completion / failure (parses the code from `research_error` with `^([A-Z_]+):` regex and routes to `/deck/error/<code>`). Slot-replaces the failed-attempt `vs-generate-deck` in production.
